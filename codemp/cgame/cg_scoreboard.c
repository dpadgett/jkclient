...
static void CG_DrawClientScore( int y, score_t *score, float *color, float fade, qboolean largeFormat )
{
	...
	if ( score->ping != -1 )
	{
		...
		if ( cg_scoreboardBots.integer && ci->botSkill != -1 )
			CG_Text_Paint( SB_PING_X - CG_Text_Width( "BOT", 1.0f * scale, FONT_SMALL ) / 2, y, 1.0f * scale, colorWhite, "BOT", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );
		else
...
static qboolean CG_UseNewScoreboard() {
	return cgs.gametype == GT_CTF || cgs.gametype == GT_TEAM;
}
...
static int CG_TeamScoreboard( int y, team_t team, float fade, int maxClients, int lineHeight, qboolean countOnly )
{
	...
	if( !countOnly && CG_UseNewScoreboard() && (team == TEAM_RED || team == TEAM_BLUE ) ) {
...
qboolean CG_DrawOldScoreboard( void ) {
	...
	if ( cg.numScores > SB_MAXCLIENTS_NORMAL || CG_UseNewScoreboard() ) {
